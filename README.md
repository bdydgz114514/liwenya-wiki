...
| `/videos` `/videos/:id` | 视频库：关键词 / 系列 / 标签 / 排序 + 加载更多；详情含**播放器**（B 站有源则内嵌官方播放器，否则连本机档案转码服务实时播放原件）、摘要、逐场景理解结果时间轴、元数据、来源路径（可复制）、同系列与前后导航 |
...
站内 1206 个视频原件是 **AV1 + Opus 的 MKV**（约 170 小时、几十 GB），浏览器不能直接播放，也不适合放到 Pages 上。
因此站点的视频页用两种方式解决观看：

1. **B 站有对应稿件的视频**：直接内嵌 B 站官方播放器（数据里的 `bilibili` 字段）。
2. **其余全部视频**：连「档案转码服务」实时播放。服务脚本是 `movie/tools/site_server.py`，**同一个端口既发静态站点、也提供转码接口**：

```bash
python3 movie/tools/site_server.py --port 8898 --root site/dist \
        --archive "/path/to/李文亚世界v2.0_full_compressed"
# 站点： http://localhost:8898/     健康检查： /health     播放： /video/<编号>?t=秒
```

播放器会自动按「已保存地址 → 同源 → 同主机 8896 → 127.0.0.1:8896」的顺序探测服务，
连不上时显示提示与「指定转码服务地址」按钮（地址存在 localStorage 的 `dsh-media-base`）。

> 缩略图注意：`videos.json` 里的 `thumb` 是相对路径（`thumbs/1.jpg`），必须用 `thumbUrl()`
> 转成根绝对路径，否则在 `/videos/`、`/videos/<id>/` 这些子路径页面会解析成 `/videos/thumbs/…` 而 404。
